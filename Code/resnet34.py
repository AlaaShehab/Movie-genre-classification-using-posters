...
def run_model():
    ...
    # base_model = applications.vgg16.VGG16(weights='imagenet', include_top=False,input_tensor=Input(shape=(224, 224, 3)), pooling='none')
    # x = base_model.output
    # x = Flatten()(x)
    ...
    # output = keras.layers.Dense(num_classes, activation='sigmoid')(x)
    # model = keras.models.Model(inputs=[model.input], outputs=[output])
    # model.load_weights('cnn_model_results/models/Resnet_34_model_split_3.h5')
    
    model.compile(loss='binary_crossentropy', optimizer=Adam(), metrics=['accuracy'])

    hist = model.fit(x_tr, y_tr, batch_size=32, epochs=50, validation_data=(x_val, y_val))
    
    model.save('cnn_model_results/models/ResNet34_mode_Split3_Augmentation.h5')
